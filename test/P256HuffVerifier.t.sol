...
contract P256VerifierTest is Test {
    ...
    function setUp() public {
        verifier = HuffDeployer.config().with_evm_version("paris").deploy("P256HuffVerifier/Verifier");
    }
        ...
        returns (bool valid, uint256 gasUsed)
    {
        ...
        (bool success, bytes memory res) = verifier.staticcall(input);
    ...
    function testWrongInputLength() public {
        ...
        bytes memory input = abi.encodePacked(hash, r, s, x, y);
        (bool success, bytes memory result) = verifier.call(input);
        ...
        (success, result) = verifier.call(input);
